...
with DAG(
    dag_id='fhv_taxi_data_ingestion_v1',
    ...
    def download_data(**kwargs):
        ...
        file_name = f"fhv_tripdata_{year_month}"
        url = f"https://github.com/DataTalksClub/nyc-tlc-data/releases/download/fhv/{file_name}.csv.gz" 
    ...
    upload_to_gcs = LocalFilesystemToGCSOperator(
        ...
        src="/tmp/fhv_tripdata_{{ ds[:7] }}.csv",        # The file we unzipped
        dst="raw/fhv/fhv_tripdata_{{ ds[:7] }}.csv",  # The path inside your bucket
    ...
    load_to_bq = GCSToBigQueryOperator(
        task_id="load_to_bq",
        bucket=BUCKET,
        source_objects=["raw/fhv/fhv_tripdata_{{ ds[:7] }}.csv"],
        destination_project_dataset_table=f"{PROJECT_ID}.{BQ_DATASET}.fhv_taxi_data_2019",
        source_format='CSV',
        ...
        autodetect=False, # We will provide the schema manually
        schema_fields=[
            ...
            {'name': 'SR_Flag', 'type': 'STRING', 'mode': 'NULLABLE'},
